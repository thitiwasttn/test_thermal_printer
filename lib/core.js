...
class ThermalPrinter {
  ...
  getPrinterName() {
    return this.Interface.getPrinterName();
  }
  ...
  async execute() {
    try {
      ...
      fs.writeFileSync('C:\\Users\\Admin\\Downloads\\test.txt', this.buffer.toString('hex'));
      console.log("==============");
  ...
  async execute2(buffer) {
    try {
      console.log('===========');
      console.log(buffer.toString('hex'))
      console.log('===========');
      // return await this.Interface.execute(buffer);   
      return await null;
    } catch (error) {
      throw error;
    }
  ...
  openCashDrawer() {
    if (this.config.type === this.types.STAR) {
      ...
      // this.append(Buffer.from([0x1d, 0x72, 0x02]))
      this.append(this.printer.config.CD_KICK_5);
    }
  }

  getStatusCustom() {
    console.log('getStatusCustom()');
    this.append(Buffer.from([0x10, 0x04, 0x01]))
  }
